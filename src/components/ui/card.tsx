...
/*
 * The bordered surface used for the plan card, the invite banner, the mode
 * tiles and the memory cards.
 *
 * Restyled from the shadcn default: a hairline `border` in rule colour rather
 * than a `ring`, and no shadow at all. The design is printed matter — a card
 * is a panel ruled onto the page, not an object floating above it.
 */
function Card({
  className,
  size = "default",
...
/** Set in Spectral: a card title is a name or a heading, never a UI label. */
function CardTitle({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      data-slot="card-title"
      className={cn(
        "font-heading text-lg leading-snug font-normal group-data-[size=sm]/card:text-base",
        className
      )}
      {...props}
    />
...
function CardFooter({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn(
        "flex items-center rounded-b-xl border-t border-border p-(--card-spacing)",
        className
      )}
      {...props}
    />
